...
- **코드 블럭 / 구분선** — 빈 줄에 `` ``` `` 를 치는 순간 코드 블럭, `---` (또는 `***`) 를 치는 순간 전체 폭 구분선으로 바뀜 (Space / Enter 필요 없음). 슬래시 메뉴에서도 고를 수 있음. 코드 블럭은 모노스페이스 다크 테마에 언어별 문법 색상, 마크다운 펜스 구문 그대로 왕복.
...
- **약속** — 시각이 찍힌 항목 (미팅, 리마인더) 을 전용 `appointments` 테이블에 저장. 시간을 모르면 **종일** 로, 며칠 이어지는 일정은 **종료일** 을 붙여 넣음. 세 뷰 모두에 노출: Day 오른쪽 레일에 목록 (여러 날 일정은 `3일 중 2일째`), Week 컬럼에 칩과 이어진 막대, Month 칸에 막대와 "이 달의 일정" 목록 — 등록과 수정은 Month 에서만. 반복은 매주(13주) / 매달(12개월). Quick Throw (`⌘⇧I`) 에 Task / Appointment 탭 토글이 있어서 어느 쪽이든 다른 앱 떠나지 않고 넣을 수 있음.
...
- **할 일 이월** — 지난 7일 안에 체크 안 하고 넘어간 할 일이 있으면 오늘 상단에 배너로 뜸. 목록 확인 후 아직 유효한 것만 고르면 그 항목들이 **이동** 함 — 하위 항목까지 통째로, 원래 있던 제목(`## Work` 등) 아래로 들어가고 원래 날짜에서는 사라져서 같은 일이 두 번 세어지지 않음. 오늘 노트가 비어 있으면 가장 최근 날의 제목 구성을 먼저 깔아 줌.
- **보류 상태** — 체크박스를 우클릭하면 **미완료 / 완료 / 보류** 메뉴가 뜸. 보류는 제3의 상태 — 일부러 미뤄둔 할 일 — 로, 형광파랑으로 표시됨(마크 `- [~]`). 보류 항목은 이월 배너와 완료율에서 제외돼서, 완료로 처리하지 않고도 더 이상 재촉당하지 않게 파킹할 수 있음.
- **AI 플래너** (Day/Week 의 지팡이 아이콘) — 할 일을 한 무더기 적고 기간을 고르면 LLM 이 날짜별로 배분함. 적은 태스크만 배치하고(과거 미완료 할 일을 끌어오지 않음), 답이 계획을 실제로 바꿀 때만 한두 개 되묻고, 노트에 반영하기 전에 자유 피드백으로 다듬을 수 있음. 적용된 계획은 날짜별 `## 📋 Plan` 섹션으로 들어가고, 다시 돌리면 그 섹션을 교체하되 이미 체크한 항목은 남김. 대화 중 네트워크가 끊겨도 초안을 버리지 않고 다시 시도 배너를 띄움. 아래 [AI 플래너](#ai-플래너) 참고.
...
- **복사 / 붙여넣기** — Notion, Google Docs, 웹, 터미널, VS Code 에서 가져온 목록의 위계와 체크박스가 그대로 들어옴. 밖으로 복사하면 들여쓰기가 살아있는 마크다운. 서식 없이 넣으려면 `⌘⇧V`.
- **자동 백업** — 켜져 있는 동안 하루 한 번 `~/Library/Application Support/Dayflow/backups/` 에 전체 스냅샷, 최근 14개 보관. Settings → 데이터 에서 Finder 로 열 수 있음.
...
- **테마** — 미드나잇 / 그래파이트 / 네이비 (어두운) 와 페이퍼 / 세피아 (밝은) 다섯 가지. 상단 바 팔레트 버튼이나 Settings 에서 바로 바꿈. 에디터·메뉴·창 외관이 전부 같이 바뀌고, 코드 블럭은 밝은 테마에서도 어둡게 유지돼서 문법 색상이 읽힘.
- **이중 언어 지원** — 영어, 한국어. Settings 에서 전환 가능. 메뉴바, 에디터 슬래시 메뉴, 상태 메뉴까지 같이 바뀜.
- **단축키** — `⌘1` / `⌘2` / `⌘3` 으로 Day / Week / Month, `⌘←` / `⌘→` 이전·다음, `⌘T` 오늘, `⌘⇧F` 전역 검색, `⌘⇧V` 서식 없이 붙여넣기.

## 화면

### Day 뷰
- 왼쪽은 마크다운 에디터. 오른쪽 레일 위에서부터:
    - 이번 주 7일 링 — 날마다 완료 비율만큼 채워지고, 누르면 그 날로 이동.
    - 오늘 진행 — 전체 완료 수와, 노트의 **제목(`## …`)별 진행 막대**. 제목 줄을 누르면 에디터가 그 제목으로 스크롤.
    - 약속 (여러 날 일정 포함), **보류 중** 인 할 일 (지난 30일), AI 회고. 비어 있는 항목은 한 줄로 접힘.
- 체크리스트, 메모, 중첩 목록이 하루 하나의 본문 안에 모두 들어감.
- 지난 날에 안 끝낸 할 일이 남아 있으면 상단에 배너가 뜸 — [할 일 이월](#할-일-이월) 참고.
- 상단 툴바: **B** / *I* / <u>U</u> / ~~S~~ / `{ }` + 글자색 / 형광펜. 드래그로 선택 후 버튼 클릭. 선택이 없을 땐 흐려져서 글에 집중되고, 선택하면 다시 선명해짐. 색 스와치는 두 버튼 뒤에 접혀 있고, 각 버튼 밑줄이 지금 선택에 적용된 색을 보여줌.
- 슬래시 메뉴 (빈 줄에서 `/`): 헤딩, 목록, 코드 블럭, 표 등.

![Day 뷰](Dayflow-macOS/docs/screenshots/ko/day.png)

### Week 뷰
- 7개 컬럼, 요일별로 하나씩. 여러 날 일정은 컬럼을 가로지르는 **이어진 막대**로, 그 아래 그날의 약속 칩, 그 아래 제목별로 묶인 할 일.
- 할 일 제목은 두 줄까지 보이고 마우스를 올리면 전체가 뜸. 할 일이 많은 날은 그 컬럼 안에서만 스크롤.
- 체크박스는 현장 토글 — 박스를 눌러도 Week 뷰에서 벗어나지 않고, 그날 노트의 색·밑줄도 그대로 유지됨.

![Week 뷰](Dayflow-macOS/docs/screenshots/ko/week.png)

### Month 뷰
- 일일 활동량에 따라 색이 진해지는 히트맵. 칸마다 안 끝난 할 일 미리보기와 `완료/전체` 막대, 마우스를 올리면 그날 할 일 전체.
- 여러 날 일정은 칸을 가로지르는 막대로, 기간이 겹치면 줄을 나눠서 표시.
- 오른쪽 레일 위에서부터: 월간 메트릭 (완료율, 최장 연속, 가장 활발한 요일), 이 달의 일정 전체, **이달 계획** 에디터 (월 단위 TODO).
- 일정 등록은 Month 뷰에서 함. 추가 폼은 **일정 추가** 버튼 뒤에 접혀 있음 — 레일보다 넓기도 하고, 일정 등록은 늘 하는 일이 아니니까. 폼은 지금 보고 있는 날짜로 열리고, **종일** 과 **+ 종료일** 버튼이 있음. 각 줄의 연필을 누르면 그 일정이 채워진 채로 열리고, × 는 확인 후 삭제.
...
- 각 줄에 원래 있던 제목과 함께 옮겨질 하위 항목 수가 보임.
- 확정하면 할 일이 하위 항목까지 **이동** 함 — 오늘 노트의 같은 제목 아래로 들어가고 (없으면 제목을 만들어 줌), 원래 날짜에서는 지워짐. 글자색 같은 서식도 따라감. 시트를 열어둔 사이에 원본 날짜가 바뀌었다면 그 블록은 건드리지 않고 건너뜀.

![할 일 이월](Dayflow-macOS/docs/screenshots/ko/carryover.png)

### 테마
- 상단 바의 팔레트 버튼, 또는 Settings → 일반 → 표시 에서 고름. 아래는 밝은 **페이퍼** 테마.

![페이퍼 테마](Dayflow-macOS/docs/screenshots/ko/day-paper.png)

### AI 플래너
- 시트 하나, 세 단계: **할 일 덤프 + 기간 선택 → (선택) 질문 → 프리뷰 → 적용**. Day/Week 네비게이션 바의 지팡이 아이콘으로 열림. Day 는 선택한 날, Week 는 보이는 주가 기본값으로 채워짐.
- 적은 태스크만 배치됨. 기존 약속은 *제약*(사용 불가 시간) 으로만 넘어가고, 과거 미완료 할 일이나 이달 계획 목표는 일부러 뺌 — 그래서 계획은 딱 요청한 것만 담김.
- 모델은 **답이 계획을 바꿀 때만** 되물음(우선순위, 마감, 소요 시간). 답하거나, *그냥 이대로 짜줘* 를 누르면 합리적 가정으로 진행함(가정은 rationale 에 밝힘). 최대 2라운드.
- 프리뷰는 대화로 다듬을 수 있음: 자유 피드백(*"블로그 글을 더 앞으로", "목요일은 비워줘"*) 을 적고 **다시 짜줘** 를 누르면 같은 대화 안에서 원하는 만큼 재계획함, 적용 전까지.
- **적용** 하면 각 날짜 노트에 `## 📋 Plan (…)` 섹션이 들어감. 그 날에 대해 다시 돌리면 섹션을 교체하되 이미 체크한 항목은 보존함. 적용을 누르기 전까지는 아무것도 안 씀.
- 턴은 트랜잭션 방식: 대화 중 네트워크가 끊겨도 진행 상태를 유지하고 초안을 버리는 대신 **다시 시도** 배너를 띄움.
...
### Settings
- **일반** — *표시* (앱 언어, 테마, 에디터 글자 크기, 공휴일 표시 — 한국 / 미국 / 둘 다, 앱에 내장되어 네트워크 안 씀), *알림* (약속 알림과 미리 알림 시각), *데이터* (Dayflow 를 쓰기 시작한 날짜, 자동 백업 — 마지막 백업 시각과 Finder 로 열기) 로 묶여 있음.
