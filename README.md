...
- **Code blocks and dividers** — typing `` ``` `` on an empty line turns it into a code block, and `---` (or `***`) into a full-width divider, the moment the line reads that way — no Space or Enter. Both are in the slash menu too. Code blocks are monospaced, dark-themed, syntax-highlighted per language, with fenced markdown round-tripping.
