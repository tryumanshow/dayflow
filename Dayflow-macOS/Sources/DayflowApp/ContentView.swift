...
struct ContentView: View {
    ...
    private var headerLabel: String {
        ...
        case .day:
            return DF.koFullDate.string(from: store.selectedDate)
        case .week:
            let start = store.startOfWeek(store.selectedDate)
            let end = Calendar.current.date(byAdding: .day, value: 6, to: start) ?? start
            return "\(DF.koShortMonthDay.string(from: start)) – \(DF.koShortMonthDay.string(from: end))"
        case .month:
            return DF.koMonthTitle.string(from: store.selectedDate)
        }
    }
    ...
    private var daySummaryRail: some View {
        ...
        return VStack(alignment: .leading, spacing: DS.Space.sm) {
            ...
            if total > 0 {
                Text("\(counts.done) done · \(counts.open) open")
    ...
    private var weekView: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: DS.Space.lg) {
                Spacer()
                weekFooterStat(value: "\(totals.done)", label: "done")
                weekFooterStat(value: "\(totals.open)", label: "open")
                weekFooterStat(value: "\(totals.trackedDays)", label: "days tracked")
                Spacer()
            }
            .padding(.vertical, DS.Space.md)
    ...
    private func weekColumn(for day: Date) -> some View {
        ...
        return VStack(alignment: .leading, spacing: DS.Space.md) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text(DF.koWeekday.string(from: day).uppercased())
    ...
    private func monthMetricsRail(_ stats: DayflowStore.MonthStats) -> some View {
        VStack(alignment: .leading, spacing: DS.Space.sm) {
            SectionLabel(text: DF.koMonthTitle.string(from: store.selectedDate))
            HStack(alignment: .bottom, spacing: 4) {
    ...
    private func monthCaption(_ stats: DayflowStore.MonthStats) -> String {
        var parts: [String] = []
        parts.append("\(stats.doneTasks) of \(stats.totalTasks)")
        if stats.longestStreak > 0 {
            parts.append("\(stats.longestStreak)일 연속")
        }
        if let weekday = stats.busiestWeekday {
            parts.append("가장 활발: \(weekday)요일")
        }
    ...
    private var selectedDayPreviewRail: some View {
        let dateLabel = DF.koShortDate.string(from: store.selectedDate)
    ...
    private func humanDateLabel(from ymd: String) -> String {
        ...
        return DF.koShortDate.string(from: d)
    }
}
